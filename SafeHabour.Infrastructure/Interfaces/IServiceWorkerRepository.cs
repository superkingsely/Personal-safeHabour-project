...
public interface IServiceWorkerRepository
{
    ...
    Task<ServiceWorkerDto?> GetServiceWorkerByUserIdAsync(Guid userId);
    ...
    /// <returns>ServiceWorkerDto if found, null otherwise</returns>
    Task<ServiceWorkerDto?> GetServiceWorkerByIdAsync(int serviceWorkerId);
    ...
    Task<ServiceWorkerDto?> UpdateServiceWorkerAsync(UpdateServiceWorkerRequest request, string? profilePicturePath = null);
    ...
    Task<PaginatedResponse<ServiceWorkerSearchResultDto>> SearchServiceWorkersAsync(SearchServiceWorkersRequest searchRequest, Guid? currentUserId = null);
}
